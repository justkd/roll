...
class Scaled {
    ...
    constructor(value, range = [0, 1]) {
        const notNumber = (v) => {
            ...
            return shouldParse ? parseFloat(`${v}`) : v.value();
        };
        let v = typeof value === "number" ? value : notNumber(value);
        let r = range;
    ...
    static round(value, places = 0) {
        return Scaled.floatingPointFix(Number(`${Math.round(Number(`${value}e${places}`))}e-${places}`));
    }
    /**
     * @static
